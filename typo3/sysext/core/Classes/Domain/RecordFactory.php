...
class RecordFactory
{
    public function createFromDatabaseRecord(string $table, array $record): Record
    {
        ...
        $fullType = GeneralUtility::underscoredToUpperCamelCase($table);
        if ($fullType === 'TtContent') {
            $fullType = 'Content';
        }
        if ($typeFieldName !== null) {
            unset($properties[$typeFieldName]);
            $fullType .= '.' . GeneralUtility::underscoredToUpperCamelCase($typeValue ?? 'default');
        }
        $obj = Record::createFromPreparedRecord($properties, $fullType, $specialProperties);
        return $obj->withRawProperties($record);
    }

    protected function findRelevantFieldsForSubSchema(array $tcaForTable, ?string $subSchemaName): array
    {
        $fields = [];
        $subSchemaConfig = $tcaForTable['types'][$subSchemaName ?? '1'] ?? $tcaForTable['types'][1] ?? $tcaForTable[0];
     ...
     */
    protected function getFinalFieldConfiguration($fieldName, array $schemaConfiguration, array $subSchemaConfiguration, ?string $fieldLabel): array
    {
